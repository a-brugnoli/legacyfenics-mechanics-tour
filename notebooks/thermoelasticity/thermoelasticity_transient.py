...
folder_results = Path(parent / "results")
folder_results.mkdir(exist_ok=True, parents=True)
filename = folder_results / "thermoelasticity_results.xdmf"
file_results = XDMFFile(str(filename))
file_results.parameters["flush_output"] = True
file_results.parameters["functions_share_mesh"] = True

file_results.write(u, 0.)
file_results.write(Theta, 0.)
...
for (i, dti) in enumerate(np.diff(t)):
    ...
    u, Theta = U.split()
    file_results.write(u, t[i+1])
    file_results.write(Theta, t[i+1])
...
plt.ylabel(r"Temperature variation $\Theta$")
plt.legend([r"$t={:.0f}$".format(ti) for ti in t[1::Nincr//10]], ncol=2)
plt.show()
...
plt.figure()
p = plot(sigma(u, Theta)[1, 1], title=r"$\sigma_{yy}$ stress near the hole")
plt.xlim((0, 3*R))
plt.ylim((0, 3*R))
plt.colorbar(p)
plt.show()
